...
import wandb
wandb_key = "f5f5d955d4c8d475146a213b0b1c798818822837"  # scy
assert (wandb_key is not None)
wandb.login(key=wandb_key)

run_name=time.strftime('%Y_%m_%d_%H:%M:%S')
gp_name="DeiT-S-fp"
wandb.init(project="BinaryViT",name=run_name,group=gp_name)
...
def main(args):
    ...
    if args.teacher_model:
        ...
        # for k,v in best_teacher_model.items():
        #     print(k)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        test_stats = evaluate(data_loader_val, model, device)
        # wandb.log({{f'train_{k}': v} for k, v in test_stats.items()})
